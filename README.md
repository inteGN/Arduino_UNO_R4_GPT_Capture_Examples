...
1. **input-capture example** `gpt_capture_example.ino`  
   - Arduino UNO R4 MINIMA (RA4M1)でGPT2タイマーを使用して入力パルスをキャプチャする方法を示します。  
   - D4/GPT2_Aピンの各立ち上がりエッジにはタイムスタンプが付けられ、キャプチャされた値はシリアルモニターを通じて表示されます。  
   - 検証のための簡単な入力ソースを提供するため、PWMテスト信号がD6ピンに生成されます。  
   - It demonstrates how to capture input pulses on the Arduino UNO R4 MINIMA (RA4M1) using the timer GPT2.  
